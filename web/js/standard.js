...
return;
		...
		function dateSql2German(date,full=true) {
...
function getOwnBanner() {
	console.log('getOwnBanner');
	var srcString = '<div><iframe id="frameBanner" style="width:100%" frameBorder=0></iframe></div>';
	$("#srcBanner").html(srcString);
	var data={
			"kind": "logo",
	...
	$('#frameBanner').attr('src',"/getFirmFile?data="+data);
...
function hideBanner() {
	...
	$('#showBanner').css("display","table");
	$('#footerCompany').css("visibility","visible");
	$('#footerUser').css("visibility","visible");
	
}
function showBanner() {
	$('#logo').css("display","table");
	$('#showBanner').css("display","none");
	$('#footerCompany').css("visibility","hidden");
	$('#footerUser').css("visibility","hidden");
}
	...
	var string="";
	string=tbodyRowsEmbedded($('#account').html());
	  $('#account').html(string);
	  $('#account').css("display","table");
	  string=tbodyRowsEmbedded($('#footer').html());
	  $('#footer').html(string);
		return;
	}
	...
	function showIconMenu(pmenu='icon_menu') {
	console.log(pmenu);
	console.log($('#'+pmenu).html());
	
	if ($('#'+pmenu).css('display')=='table') {
		$('#'+pmenu).css('display','none');	  
	} else {
		var y=window.innerWidth;
		$('#'+pmenu).css('right',10);	  
		$('#'+pmenu).css('display','table');
		$('#'+pmenu ).menu();
	}
	return;
	} ;
...
var $frm = $("form#"+pform);	
	...
	switch (ifield) {
		case "classID":
				...
				$frm.find('select[name="userID"]').append( new Option('------',0) );
...
// Dialogfunction
function dialogOpen(dialogID,obj='') {
	$(dialogId).dialog(
			{
				buttons : {
					Close : function() {
...
function dialogInit(dialogId,obj) {
	...
	if (dialogId.find('select[name="termPayment"]')!=undefined) {
		arraySelect = [ 
			{
				"field": "termPayment",
...
function dialogInitSelect(dialogId,array) {
	...
	if (dialogId==undefined || dialogId=="") {
		alert("dialogId undefined or empty");
		return;
	}
	
	if (array==undefined || array.length==0)
		alert("array undefined or length 0");
...
function dialogInitSelectG(dialogId,array) {
	...
	var source=new Array();
		...
		dialogId.find('select[name="'+field+'"]').find('option').remove().end();
		dialogId.find('select[name="'+field+'"]').find('optgroup').remove().end();	
		/*
		 * collect groups
		 * 
		 */
	    ...
	    for (var gi=0;gi<arrayGroup.length;gi++) {
            ...
            for (var i=0;i<rows.length;i++) {
            	if (eval('rows[i].'+groupField)==group) {
            		found=false;
            		if (arrayUser.indexOf(eval('rows[i].'+label))>=0) {
    					found=true;
    				}
            		
            		if (!found) {
            			
            			arrayUser.push(eval('rows[i].'+label));
            			var option = $("<option style='font-size:10;'></option>");
            ...
            dialogId.find('select[name="'+field+'"]').append(optgroup);
	    }
...
function dialogDeleteId(dialogId,id,modul,postFunction = '') {
	...
	if (id==undefined || id=='') {
		...
		alert('programer, expected id for '+modul);
		return;
	}
...
function dialogSave(dialogId,id,modul,postFunction = '',data=new Array()) {
	...
	if (data.length==0) {
		...
		dialogId.find('textarea').each(function () {
			var textareaField='textarea[name="'+this.name+'"]';
			if (tinyMCE.activeEditor.getContent()!=undefined) {
				data[this.name]=tinyMCE.activeEditor.getContent();
			} else {
				data[this.name]=dialogId.find(textareaField).html();	
			}
...
function innerHTMLInit(htmlId,obj) {
	if (obj==undefined || obj=="")
		alert("innerHTMLInit in standard.js kein obj");
	console.log(obj);
...
function tbodyRows(htmlId,tbody,rows,filter="",insert=true) {
	...
	if (htmlId==undefined || htmlId=="") {
		alert("1st Parameter htmlId not defined");
		console.log('htmlId: '+htmlId);
		console.log('tbody: '+tbody);
		console.log('isArray rows: '+Array.isArray(rows));
		console.log(rows);
		return;
	}

	if (tbody==undefined || tbody=="") {
		alert("2nd Parameter tbody not defined");
		console.log('htmlId: '+htmlId);
		console.log('tbody: '+tbody);
		console.log('isArray rows: '+Array.isArray(rows));
		console.log(rows);
	...
	if (rows==undefined) {
		alert("3rd Parameter rows not defined\n if not defined\n obj.dataRowsName='rowsOrderPositions';");
		console.log('htmlId: '+htmlId);
		console.log('tbody: '+tbody);
		console.log('isArray rows: '+Array.isArray(rows));
		console.log(rows);
	...
	if (tbody.indexOf('[')<0) {
		alert("no [fielddefinition] in tbody "+htmlId);
		console.log('htmlId: '+htmlId);
		console.log('tbody: '+tbody);
		console.log('isArray rows: '+Array.isArray(rows));
		console.log(rows);
	...
	if (!Array.isArray(rows)) {
		alert("rows is not an array, check Controller for _db_rows not _db_row!");
		console.log('htmlId: '+htmlId);
		console.log('tbody: '+tbody);
		console.log('isArray rows: '+Array.isArray(rows));
		console.log(rows);
...
function tbodyRowsG(htmlId,tbody,rows,groupField) {
	...
	function tbodyRowsEmbedded(temp) {
		...
		while (temp.indexOf('{')>=0 && z<100)
			...
			var value=eval(func);
			temp=temp.replace(search,value);
			}
...
function fileInternalPreview(id,form,division) {
	var dialogId=$("form#dialogFiles");
	for (var i=0;i<array_data["rowsFilesInternal"].length;i++) {
		if (array_data["rowsFilesInternal"][i].printForm!=undefined) {
			var printForm=array_data["rowsFilesInternal"][i].printForm;
			var printLang=array_data["rowsFilesInternal"][i].printLang;
			break;
		}
	}
	
	var cb_form="cb_"+form;
	...
	data=JSON.stringify(data);
	$('#previewFile').attr('src',"/"+url+"?data="+data);
...
function showHelp(firmID=-1) {
	if (firmID==-1) 
		firmID=_session.firmID;
	$("#dialogHelp").dialog(
			{
				title : "Hinweise zum portalPlus",
				width: 1200,
				height: 400,
				buttons : {
					Close : function() {
...
function checkFirmID(company,warning=true) {
	if (company=='' && warning) {
	...
	if (firmID==-1 && warning) 
		alert('Firma: "'+company+'" nicht gefunden!');
...
class _Position {
	  ...
	  _refresh() {
		  ...
		  if (this.rowsPositions==undefined) {
			  alert("expected: "+this.dataRowsName+"\nshould come from controller PositionSave()\ntry with rows coming from Controller");
			  if (array_data["rows"]==undefined) {
				  alert("also now rows coming from Controller\nhave a look in console.log\nshows complete available array_data");
				  console.log(array_data)
			  } else {
				  this.rowsPositions=array_data["rows"];
			  }
		  }
...
function totalSum(dialogId,rows) {
	...
	var row=new Array();
	row["merchandiseNetVn"]=germanDezimal(number2String(merchandiseNetVn)); 
	row["merchandiseNetVr"]=germanDezimal(number2String(merchandiseNetVr)); 
	row["packageNetVn"]=germanDezimal(number2String(packageNetVn)); 
	row["packageNetVr"]=germanDezimal(number2String(packageNetVr)); 
	row["dispatchNetVn"]=germanDezimal(number2String(dispatchNetVn)); 
	row["dispatchNetVr"]=germanDezimal(number2String(dispatchNetVr)); 
	row["NetVn"]=germanDezimal(number2String(NetVn)); 
	row["NetVr"]=germanDezimal(number2String(NetVr)); 
	row["VATn"]=germanDezimal(number2String(VATn)); 
	row["VATr"]=germanDezimal(number2String(VATr)); 
	row["grossValue"]=germanDezimal(number2String(grossValue)); 
	row["tradeDiscount"]=germanDezimal(number2String(tradeDiscount)); 
	row["paymentAmount"]=germanDezimal(number2String(paymentAmount)); 
...
function isHtmlContent(modul) {
	var content;
	for (var i=0;i<_htmlContent.length;i++) {
		if (_htmlContent[i].modul==modul) {
			content=_htmlContent[i].content;
			break;
		}
	}
	return content;
...
function isGridContent(modul) {
	var content;
	for (var i=0;i<_htmlContent.length;i++) {
		if (_gridContent[i].modul==modul) {
			content=_gridContent[i].content;
			break;
		}
	}
	return content;
